...
@Component({
    ...
    <div class="ili-gallery">
      @if (images().length > 0) {
        ...
        <div class="ili-gallery-controls">
          <button class="ili-gallery-nav" (click)="prev()" [disabled]="currentIndex() === 0">
            <app-icon icon="mdi:chevron-left" />
          </button>
          <span class="ili-gallery-counter">{{ currentIndex() + 1 }} / {{ images().length }}</span>
          <button class="ili-gallery-nav" (click)="next()" [disabled]="currentIndex() === images().length - 1">
            <app-icon icon="mdi:chevron-right" />
          </button>
        </div>

        @if (thumbnailPosition() !== "bottom") {
          <div class="ili-gallery-thumbnails" [class.ili-gallery-thumbnails-bottom]="thumbnailPosition() === 'bottom'">
            @for (image of images(); track $index) {
              <button
                ...
                (click)="goTo($index)"
              >
                <img [src]="image.thumbnail || image.src || image.url || image" alt="" />
              </button>
            }
          </div>
        }
  ...
  styles: [`
    ...
    .ili-gallery-track:active { cursor: grabbing; }
    .ili-gallery-slide {
      flex-shrink: 0;
      display: flex;
      align-items: center;
    ...
    .ili-gallery-thumbnails-bottom { order: 1; }
    .ili-gallery-thumb {
      flex-shrink: 0;
      width: 3.5rem;
      height: 3.5rem;
