...
import { IonButton, IonCard, IonCardContent, IonCardHeader, IonCardTitle, IonProgressBar } from "@ionic/angular/standalone";
import { AppIcon } from "@ui/shared/app-icon/app-icon.component";
import { MobileEmptyState } from "@ui/mobile/empty-state/empty-state";
import { TreeTableBase, TreeNodeBase } from "@ui/base/tree-table.base";

@Component({
  selector: "ili-tree-table",
  standalone: true,
  imports: [CommonModule, IonButton, IonCard, IonCardContent, IonCardHeader, IonCardTitle, IonProgressBar, AppIcon, MobileEmptyState],
  template: `
      ...
      <ng-template #nodeTemplate let-node let-depth="depth">
        <div class="ili-tree-node" [style.paddingLeft.px]="depth * 20 + 8">
          <div class="ili-tree-node-row" (click)="toggleNode(node)">
            @if (!node.leaf) {
              <ion-button fill="clear" size="small" class="ili-tree-node-toggle">
                <app-icon [icon]="node.expanded ? 'mdi:chevron-down' : 'mdi:chevron-right'" />
              </ion-button>
            } @else {
            ...
            <span class="ili-tree-node-label" [class.ili-tree-node-selectable]="node.selectable !== false" (click)="selectNode(node)">
  ...
  `,
  styles: [`
    .ili-tree-table-root { width: 100%; }
    .ili-tree-table-header {
      padding: 0.75rem 0.5rem;
      font-size: var(--ds-font-size-section-title, 1rem);
      color: var(--ds-text-primary);
    }
    .ili-tree-table-nodes { padding: 0.25rem 0; }
    .ili-tree-node { padding: 0.25rem 0; }
    .ili-tree-node-row {
      display: flex;
      align-items: center;
      gap: 0.25rem;
    ...
    .ili-tree-node-toggle { --padding-start: 0; --padding-end: 0; min-height: 1.5rem; }
...
export class MobileTreeTable extends TreeTableBase {
  ...
  selectNode(node: TreeNodeBase): void {
    ...
    if (this.selectionMode() === "single") {
      ...
      const exists = current.find((s: any) => (s.key || s.data) === (node.key || node.data));
      if (exists) {
        this.selection.set(current.filter((s: any) => (s.key || s.data) !== (node.key || node.data)));
