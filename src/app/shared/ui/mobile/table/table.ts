...
import { IonCard, IonCardContent, IonCardHeader, IonCardTitle, IonList, IonItem, IonLabel, IonProgressBar } from "@ionic/angular/standalone";
...
@Component({
  selector: "ili-table",
  standalone: true,
  imports: [CommonModule, IonCard, IonCardContent, IonCardHeader, IonCardTitle, IonList, IonItem, IonLabel, IonProgressBar, AppIcon, MobileEmptyState],
  template: `
              ...
              @for (col of columns(); track col.field) {
                @if (!$first) {
                  <div class="ili-table-field">
                    ...
                    <span class="ili-table-field-value">{{ row[col.field] }}</span>
                  </div>
                }
              }
            </ion-card-content>
  ...
  styles: [`
    ...
    .ili-table-card-icon { font-size: 1.25rem; }
    ...
    .ili-table-field:last-child { border-bottom: none; }
