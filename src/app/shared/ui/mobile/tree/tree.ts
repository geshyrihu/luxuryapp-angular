...
@Component({
      ...
      <div class="ili-tree-node" [style.paddingLeft.px]="depth * 20 + 8">
        <div class="ili-tree-node-row" (click)="selectNode(node)">
          @if (!node.leaf && node.children?.length) {
            <button class="ili-tree-node-toggle" (click)="toggleNode(node); $event.stopPropagation()">
          ...
          @if (selectionMode() === "checkbox") {
            <button class="ili-tree-checkbox" (click)="toggleCheck(node); $event.stopPropagation()">
              <app-icon [icon]="isChecked(node) ? 'mdi:checkbox-marked' : (isPartialChecked(node) ? 'mdi:minus-box' : 'mdi:checkbox-blank-outline')" />
            </button>
          }
          ...
          <span class="ili-tree-node-label" [class.ili-tree-node-selected]="isSelected(node)">
            {{ node.label }}
          </span>
        </div>
  ...
  styles: [`
    .ili-tree { width: 100%; }
    .ili-tree-node { padding: 0.125rem 0; }
    .ili-tree-node-row {
      display: flex;
      align-items: center;
      gap: 0.25rem;
...
export class MobileTree extends TreeBase {
  ...
  isSelected(node: TreeNode): boolean {
    ...
    if (Array.isArray(sel)) return sel.some((s: any) => (s.key || s.label) === (node.key || node.label));
    return false;
  }
  ...
  selectNode(node: TreeNode): void {
    ...
    if (this.selectionMode() === "single") {
      ...
      const current: any[] = Array.isArray(this.selection()) ? this.selection() : [];
      const exists = current.some((s: any) => (s.key || s.label) === (node.key || node.label));
      if (exists) {
        this.selection.set(current.filter((s: any) => (s.key || s.label) !== (node.key || node.label)));
      } else {
  ...
  isPartialChecked(node: TreeNode): boolean {
    ...
    const someChecked = node.children.some((c) => this.checked().has(c) || this.isPartialChecked(c));
  ...
  private toggleChildrenCheck(node: TreeNode, checked: boolean, set: Set<TreeNode>): void {
