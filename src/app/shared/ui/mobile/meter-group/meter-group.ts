...
@Component({
        ...
        @for (item of value(); track $index) {
          <div class="ili-meter-group-legend-item">
            <span
              ...
              [style.background]="item.color || 'var(--ds-primary, #003d9b)'"
            ></span>
            <span class="ili-meter-group-legend-label">{{ item.label || '' }}</span>
  ...
  styles: [`
    ...
    .ili-meter-group-segment:first-child {
      border-radius: var(--ds-radius-full, 9999px) 0 0 var(--ds-radius-full, 9999px);
    }
    .ili-meter-group-segment:last-child {
      border-radius: 0 var(--ds-radius-full, 9999px) var(--ds-radius-full, 9999px) 0;
    }
...
export class MobileMeterGroup extends MeterGroupBase {
  segmentWidth(item: any): number {
    const total = this.value().reduce((sum: number, i: any) => sum + (i.value ?? 0), 0);
    if (total === 0) return 0;
