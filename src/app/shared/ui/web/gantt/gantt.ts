...
export class AppGantt {
  ...
  private earliestDate(): Date {
    ...
    return new Date(
      Math.min(...this.tasks().map((t) => t.startDate.getTime())),
    );
  }
  ...
  barLeft(task: GanttTask): number {
    const diff = Math.max(
      0,
      (task.startDate.getTime() - this.viewStart().getTime()) / 86400000,
    );
  ...
  barTooltip(task: GanttTask): string {
    ...
    const prog =
      task.progress !== undefined ? ` — ${task.progress}% completado` : "";
