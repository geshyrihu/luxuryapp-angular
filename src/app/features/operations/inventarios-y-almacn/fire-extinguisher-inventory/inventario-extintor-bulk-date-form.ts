...
@Component({
  ...
  imports: [ReactiveFormsModule, CardModule, CustomInputDateSignal, CustomButtonSave],
})
export class InventarioExtintorBulkDateForm {
  ...
  form: FormGroup<IBulkDateForm> = this.formB.group({
    expirationDate: new FormControl<string | null>(null, { validators: [Validators.required] }),
  });
  ...
  onSubmit() {
    ...
    this.apiResponseS
      .onPut(`InventarioExtintor/bulk-expiration/${this.customerIdS.customerId()}`, {
