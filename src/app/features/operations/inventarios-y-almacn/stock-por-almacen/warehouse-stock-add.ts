...
export class WarehouseStockAdd implements OnInit {
  ...
  // ConfiguraciÛn de paginaciÛn y filtro
  rows: number = 30; // Registros por p·gina
  first: number = 0; // √çndice del primer registro
  ...
  async onSubmit(rowGroup: FormGroup<IWarehouseStockRowForm>) {
    ...
    // Validar que Stock MÛnimo < Stock MÛximo
    if (value.stockMax && value.stockMin && value.stockMin >= value.stockMax) {
      rowGroup.controls.errorMessage.setValue(
        "El 'Stock MÛnimo' no puede ser mayor o igual al 'Stock MÛximo'.",
      );
      return;
    }
