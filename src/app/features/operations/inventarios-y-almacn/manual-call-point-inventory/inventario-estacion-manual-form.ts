...
export class InventarioEstacionManualForm implements OnInit {
  ...
  form: FormGroup<IInventarioEstacionManualForm> = this.formB.group({
    ...
    stationType: new FormControl<number | null>(null, { validators: [Validators.required] }),
  ...
  async ngOnInit() {
    this.cb_stationType = await firstValueFrom(this.enumSelectS.manualStationType());
  ...
  onLoadData() {
    this.apiResponseS.onGetItem(`InventarioEstacionManual/${this.id}`).then((result: any) => {
  ...
  onSubmit() {
    ...
    if (!this.id) {
      this.apiResponseS.onPost("InventarioEstacionManual", formData)
        .then((result: boolean) => { result ? this.ref.close(true) : this.submitting.set(false); });
    } else {
      this.apiResponseS.onPut(`InventarioEstacionManual/${this.id}`, formData)
        .then((result: boolean) => { result ? this.ref.close(true) : this.submitting.set(false); });
    }
  }
