...
export class AnnouncementAdminForm implements OnInit {
  ...
  ngOnInit() {
    ...
    // Cargar catálogos siempre, luego datos si es edición
  ...
  async loadDataForEdit() {
    ...
    if (announcementData) {
      ...
      // Preparar objeto para patchValue (excluyendo archivos y lígica especial)
      ...
      // Mapear Status y Type de string a numírico (si vienen como texto)
      ...
      // Si el backend devuelve strings, los convertimos. Si devuelve números, usamos directo.
      const statusVal =
  ...
  async generateDraft() {
    const { value: prompt } = await this.swalService.fire({
      ...
      input: "textarea",
      inputLabel: "óQué quieres comunicar?",
