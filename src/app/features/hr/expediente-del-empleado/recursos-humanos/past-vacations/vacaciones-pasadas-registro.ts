...
} from "src/app/features/hr/expediente-del-empleado/recursos-humanos/interfaces/register-past-vacation.interface";
...
export class VacacionesPasadasRegistro implements OnInit {
  ...
  onLoadData() {
    ...
    this.form.controls.dateRange.valueChanges
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((value) => {
        console.log("ü‚Äù¬ç [ValueChanges] dateRange:", value);
        this.dateRangeSignal.set(value);
        this.form.updateValueAndValidity();
      });
  }

  // Nuevo mÛtodo para capturar la selecciÛn manual
  onDateSelect(value: Date[] | null) {
    console.log("click ?? [onSelect] Calendario:", value);
    // Forzamos la actualizaciÛn por si el ValueChanges falla
    this.dateRangeSignal.set(value);
    this.form.updateValueAndValidity();
    console.log(
      "ü‚Äù¬ç Estado Form tras selecciÛn:",
      this.form.status,
      "Errores:",
      this.form.errors,
    );
  ...
  loadBalance(employeeId: number): void {
    ...
    this.apiResponseS
      ...
      .then((response: VacationBalanceDTO) => {
        console.log("ü‚Äù¬ç Balance Cargado:", response);
        ...
        if (response) {
          ...
          if (monthsOfService < 6) {
            console.warn("‚ö†Ô∏¬è Empleado con menos de 6 meses");
            ...
            console.warn("‚ö†Ô∏¬è No elegible este AÒo");
  ...
  dateRangeValidator(control: AbstractControl): ValidationErrors | null {
    ...
    console.log(
      `ü‚Äù¬ç Validando: Solicitados=${requestedDays}, Disponibles=${available}`,
    );

    if (requestedDays > available) {
      console.error("? Error: DÌas insuficientes");
      return {
        notEnoughDays: {
    ...
    console.log("? ValidaciÛn exitosa");
    return null;
  }
      ...
      .then((result) => {
        if (result) {
          ...
          if (currentEmployeeId) {
            console.log(
              "ü‚Äù¬ç Refrescando datos para el empleado:",
              currentEmployeeId,
            );
            this.refreshBalance();
          ...
          this.serverError.set(
            error?.message || "OcurriÛ un error inesperado.",
          );
        }
      })
