...
export class FireInspectionPeriodEstacionDetail implements OnInit, OnDestroy {
  ...
  mergedItems = computed(() => {
    ...
    return this.periodItems().map((p) => {
      const cycleItem = cycleItems.find((c) => c.equipmentId === p.stationId);
      return { ...p, cycleStatus: cycleItem?.status ?? null, inspectedAt: cycleItem?.inspectedAt ?? null };
    });
  });

  availableEquipment = computed(() => {
    const assigned = new Set(this.periodItems().map((p) => p.stationId));
  ...
  onLoadData() {
    Promise.all([
      ...
      this.apiResponseS.onGetList(`FireInspectionPeriodItems/estacion/list/${this.periodId}`),
      this.apiResponseS.onGetItem(`FireInspectionCycle/active/${this.periodId}`),
      this.apiResponseS.onGetList(`InventarioEstacionManual/list/${this.customerIdS.customerId()}`),
  ...
  async onAddItem() {
    ...
    const result = await this.apiResponseS.onPost(
      `FireInspectionPeriodItems/estacion/${this.periodId}/${this.selectedEquipmentId}`,
      {},
    );
    if (result !== false) {
  ...
  async onRemoveItem(id: string) {
    const ok = await this.apiResponseS.onDelete(`FireInspectionPeriodItems/estacion/${id}`);
  ...
  async openChecklist(item: any) {
    ...
    await this.dialogHandlerS.openDialog(
      ...
      { cycleId: this.activeCycle().id, equipmentId: item.stationId },
      "Inspección de Estación Manual",
  ...
  private async resolveQr(rawValue: string) {
    ...
    const match = this.periodItems().find((p) => p.stationId === equipmentId);
    if (!match) {
      this.scanError.set("Este equipo no pertenece al periodo de inspección actual.");
      return;
    }
    const cycleItems: any[] = this.activeCycle()?.items ?? [];
    const cycleItem = cycleItems.find((c) => c.equipmentId === match.stationId);
