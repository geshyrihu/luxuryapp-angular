        ...
        @case ('navhub') {
          ...
          <p-card header="Navigation Hub Page — Estándar DS">
            <p class="text-sm text-secondary m-0 mb-4">
              Patrón para páginas de entrada a módulos del ERP. Consolida grupos de navegación
              en cards visuales uniformes para web y lista agrupada para mobile.
            </p>
            <p-divider />

            <!-- 1. Modelo de datos requerido -->
            <h3 class="text-base font-bold mb-2">1. Modelo de datos — <code>DashboardCard</code></h3>
            <div class="surface-ground border-round p-3 mb-4 font-mono text-xs line-height-3">
              <pre style="margin:0;white-space:pre-wrap;">{{ navHubModel }}</pre>
            </div>

            <!-- 2. Demo visual: card web -->
            <h3 class="text-base font-bold mb-2">2. Card web — patrón visual</h3>
            <div class="grid mb-4">
              @for (card of navHubDemo; track card.title) {
                <div class="col-12 sm:col-6 md:col-4 lg:col-3">
                  <div class="surface-card border-round-xl p-3 h-full shadow-1 hover:shadow-3
                    ...
                    <div class="flex align-items-center justify-content-between">
                      <div class="flex align-items-center justify-content-center border-round-lg flex-shrink-0"
                        ...
                        <app-icon [icon]="card.icon" style="font-size:1.35rem;" [style.color]="card.color" />
                      </div>
                      <app-icon icon="mdi:arrow-up-right" class="text-400 text-lg" />
                    </div>
                    <span class="font-bold text-900 text-sm line-height-2">{{ card.title }}</span>
                    @if (card.description) {
                      <p class="text-xs text-secondary m-0 line-height-3">{{ card.description }}</p>
                    }
                  </div>
                </div>
              }
              ...
              <div class="col-12 md:col-6">
                <p-card styleClass="border-round-lg p-2" header="✅ Web (≥ md)">
                  <ul class="m-0 pl-3 text-xs line-height-3">
                    <li><code>border-top: 3px solid card.color</code> (acento del grupo)</li>
                    <li>Ícono 44×44px con <code>card.bgColor</code> de fondo</li>
                    <li><code>app-icon</code> con <code>[style.color]="card.color"</code></li>
                    <li>Flecha <code>mdi:arrow-up-right</code> en gris</li>
                    <li>Label en <code>font-bold text-900 text-sm</code></li>
                    <li>Description opcional en <code>text-xs text-secondary</code></li>
                    <li>Grid: <code>col-2 xl · col-3 lg · col-4 md · col-6 sm</code></li>
                    <li>Header de grupo: barra vertical + uppercase + línea</li>
                  </ul>
                </p-card>
              </div>
              <div class="col-12 md:col-6">
                <p-card styleClass="border-round-lg p-2" header="📱 Mobile (< md)">
                  <ul class="m-0 pl-3 text-xs line-height-3">
                    <li><code>ion-list</code> con <code>ion-item-divider</code> por grupo</li>
                    <li><code>div slot="start"</code> — NUNCA <code>span</code> ni <code>ion-avatar</code></li>
                    <li>Ícono 36-38px con <code>ml-3 mr-2</code></li>
                    <li><code>[ngClass]</code> para color (aditivo) — no <code>[class]</code></li>
                    <li><code>detail="true"</code> para mostrar flecha nativa de Ionic</li>
            ...
            <h3 class="text-base font-bold mt-4 mb-2">4. Implementaciones en producción</h3>
            <div class="flex flex-wrap gap-2">
              @for (impl of navHubImplementations; track impl.route) {
                <div class="surface-ground border-round px-3 py-1 text-xs flex align-items-center gap-2">
...
interface DashboardGroup {
  ...
  readonly navHubDemo = [
    { title: 'Clientes',      icon: 'mdi:domain',               bgColor: '#dbeafe', color: '#1d4ed8', description: 'Gestión de clientes.' },
    { title: 'Roles',         icon: 'mdi:shield-account-outline',bgColor: '#e0e7ff', color: '#4338ca', description: '' },
    { title: 'Cargos',        icon: 'mdi:cash-remove',           bgColor: '#ffdad6', color: '#b91c1c', description: 'Emisión de cargos.' },
    { title: 'Pagos',         icon: 'mdi:cash-check',            bgColor: '#d1fae5', color: '#15803d', description: '' },
    { title: 'Reportes',      icon: 'mdi:chart-bar',             bgColor: '#fef9c3', color: '#a16207', description: 'Reportes contables.' },
    { title: 'Configuración', icon: 'mdi:cog-outline',           bgColor: '#f3e8ff', color: '#7c3aed', description: '' },
  ];

  readonly navHubImplementations = [
    { label: 'Configuración del Sistema', icon: 'mdi:cog',    route: '/settings/home' },
    { label: 'Contabilidad (Master)',      icon: 'mdi:wallet', route: '/contabilidad' },
