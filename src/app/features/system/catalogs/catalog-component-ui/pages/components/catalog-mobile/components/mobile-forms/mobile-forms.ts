...
@Component({
      ...
      <div class="stiich-login-card">
        ...
        <div class="stiich-login-footer">
          <div class="stiich-login-badge">
            <span class="material-symbols-outlined" style="font-size:1rem;color:#00040a;">verified_user</span>
            <div>
          ...
          <div class="stiich-login-badge">
            <span class="material-symbols-outlined" style="font-size:1rem;color:#446083;">public</span>
            <div>
  ...
  styles: [MOBILE_SHOWCASE_STYLES, `
    ...
    .stiich-login-card { background: color-mix(in srgb, var(--ds-bg-surface) 85%, transparent); backdrop-filter: blur(8px); border: 1px solid color-mix(in srgb, var(--ds-border-strong) 20%, transparent); border-radius: 0.5rem; overflow: hidden; box-shadow: 0 1px 3px color-mix(in srgb, var(--ds-text-primary) 6%, transparent); }
    .stiich-login-header { display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1rem; border-bottom: 1px solid color-mix(in srgb, var(--ds-border-strong) 20%, transparent); }
    .stiich-login-shield { font-size: 1.5rem; color: var(--ds-primary-dark, var(--ds-primary)); font-variation-settings: 'FILL' 1; }
    .stiich-login-brand { font-weight: 700; font-size: 1rem; color: var(--ds-primary-dark, var(--ds-primary)); }
    .stiich-login-body { padding: 1rem; display: flex; flex-direction: column; gap: 1rem; }
    .stiich-login-title { font-size: 1.25rem; font-weight: 700; color: var(--ds-text-primary); margin: 0; }
    .stiich-login-subtitle { font-size: 0.8125rem; color: var(--ds-text-secondary); margin: 0; line-height: 1.4; }

    .stiich-field { display: flex; flex-direction: column; gap: 0.25rem; }
    .stiich-field__label { font-size: 0.7rem; font-weight: 700; color: var(--ds-text-secondary); text-transform: uppercase; letter-spacing: 0.05em; }
    .stiich-field__forgot { font-size: 0.7rem; color: var(--ds-primary-dark, var(--ds-primary)); text-decoration: none; font-weight: 600; }
    .stiich-field__forgot:hover { text-decoration: underline; }
    .stiich-field__input-wrap { position: relative; display: flex; align-items: center; }
    .stiich-field__input { width: 100%; background: var(--ds-bg-surface); border: 1px solid color-mix(in srgb, var(--ds-border-strong) 50%, transparent); border-radius: 0.5rem; padding: 0.75rem; font-size: 0.875rem; font-family: inherit; outline: none; transition: all 150ms; }
    .stiich-field__input:focus { border-color: var(--ds-primary); box-shadow: 0 0 0 2px color-mix(in srgb, var(--ds-primary) 15%, transparent); }
    .stiich-field__pw-toggle { position: absolute; right: 0.5rem; background: none; border: none; cursor: pointer; color: var(--ds-text-muted); padding: 0.25rem; display: flex; }

    .stiich-btn-block { width: 100%; display: flex; align-items: center; justify-content: center; gap: 0.5rem; border: none; cursor: pointer; font-weight: 700; border-radius: 0.5rem; padding: 0.875rem; font-size: 0.9375rem; font-family: inherit; transition: all 150ms; }
    .stiich-btn-block--primary { background: var(--ds-primary); color: var(--ds-on-primary); box-shadow: 0 2px 4px color-mix(in srgb, var(--ds-primary) 15%, transparent); }
    .stiich-btn-block--primary:hover { background: var(--ds-primary-dark, var(--ds-primary)); }
    .stiich-btn-block--outline { background: transparent; color: var(--ds-primary); border: 1px solid var(--ds-primary); }
    .stiich-btn-block--outline:hover { background: color-mix(in srgb, var(--ds-primary) 5%, transparent); }

    .stiich-divider-with-text { display: flex; align-items: center; gap: 0.75rem; }
    .stiich-divider-with-text::before, .stiich-divider-with-text::after { content: ''; flex: 1; height: 1px; background: color-mix(in srgb, var(--ds-border-strong) 20%, transparent); }
    .stiich-divider-with-text span { font-size: 0.7rem; color: var(--ds-text-secondary); text-transform: uppercase; letter-spacing: 0.08em; font-weight: 600; }

    .stiich-login-footer { display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem; padding: 0.75rem 1rem; border-top: 1px solid color-mix(in srgb, var(--ds-border-strong) 20%, transparent); }
    .stiich-login-badge { display: flex; align-items: center; gap: 0.5rem; }
    .stiich-login-badge__label { display: block; font-size: 0.6rem; font-weight: 700; color: #00040a; text-transform: uppercase; letter-spacing: 0.04em; }
