        ...
        <p-card header="Dialogs — p-dialog">
          ...
          <div class="grid">
            ...
            <div class="col-12 md:col-6 xl:col-3">
              <div
                class="surface-ground border-round p-4 flex flex-column align-items-center gap-3 text-center"
              >
                <app-icon icon="icon.fullscreen" class="text-4xl text-primary" />
                <div>
                  <strong class="block">Grande / Fullscreen</strong>
                  <p class="m-0 text-sm text-color-secondary mt-1">
                    Para vistas complejas de solo consulta.
                  </p>
                </div>
                <p-button
                  label="Abrir"
          ...
          >
            <div class="flex flex-column gap-3">
              <div
                class="flex align-items-center gap-3 p-3 surface-ground border-round"
              >
                <app-icon
                ...
                <div>
                  ...
                  <span class="text-sm text-color-secondary"
                    >Solicitud de compra — Sistemas</span
                  >
                </div>
                <p-tag
                  value="Pendiente"
              ...
              <div class="grid text-sm">
                @for (f of detailFields; track f.label) {
                  <div class="col-6">
                    <span class="text-color-secondary block text-xs">{{
                      f.label
                    }}</span>
            ...
            <ng-template #footer>
              <p-button
                ...
                severity="secondary"
                [outlined]="true"
                (onClick)="dialogs.info.set(false)"
              />
              <p-button
                label="Editar"
                icon="icon.pencil"
          ...
          >
            <div class="flex flex-column gap-3 pt-1">
              <div>
                <label class="block text-sm font-medium mb-1"
                  >Nombre del registro *</label
                >
                <input
                  pInputText
                  [(ngModel)]="editName"
                  class="w-full"
                  placeholder="Nombre breve y auditable"
                />
              </div>
              <div>
                <label class="block text-sm font-medium mb-1"
            ...
            <ng-template #footer>
              <p-button
                ...
                severity="secondary"
                [outlined]="true"
                (onClick)="dialogs.form.set(false)"
              />
              <p-button
                label="Guardar"
          ...
          >
            <div class="flex align-items-start gap-3">
              <div
                class="flex-shrink-0 border-round p-2 mt-1"
          ...
          <!-- Dialog: Grande -->
          <p-dialog
            header="Vista Completa — Solicitudes del Mes"
            [(visible)]="dialogs.large"
            [modal]="true"
            [maximizable]="true"
          ...
          >
            ...
            <ng-template #footer>
              <p-button
                label="Cerrar"
                severity="secondary"
                (onClick)="dialogs.large.set(false)"
              />
              <p-button label="Exportar PDF" icon="icon.file-pdf-box" />
            </ng-template>
          </p-dialog>
        </p-card>
      </div>
      ...
      <div class="col-12 lg:col-6">
        <p-card header="Drawer — Panel lateral / inferior">
          ...
          <div class="flex flex-wrap gap-2">
            <p-button
              label="Drawer derecho"
              icon="icon.dock-right"
              (onClick)="drawers.right.set(true)"
            />
            <p-button
              label="Drawer izquierdo"
              icon="icon.dock-left"
              severity="secondary"
              [outlined]="true"
              (onClick)="drawers.left.set(true)"
            />
            <p-button
              label="Drawer inferior"
              icon="icon.dock-bottom"
              severity="secondary"
              [outlined]="true"
              (onClick)="drawers.bottom.set(true)"
            />
          </div>
          ...
          >
            <div class="flex flex-column gap-3">
              @for (f of drawerFilters; track f.label) {
                <div>
                  <label
                    class="block text-xs font-bold text-color-secondary uppercase mb-1"
          ...
          >
            <nav class="flex flex-column gap-1">
              @for (n of drawerNav; track n.label) {
                <button
                  class="flex align-items-center gap-3 p-3 border-round text-left w-full border-none bg-transparent text-color cursor-pointer hover:surface-hover"
                >
                  <app-icon
                    [icon]="n.icon"
                    class="text-xl flex-shrink-0 text-color-secondary"
                  />
          ...
          >
            <div class="flex flex-column gap-2 text-sm">
              <div
                class="p-3 surface-ground border-round flex align-items-center gap-3"
              >
                <app-icon
                  icon="icon.file-document-outline"
                ...
                <div>
                  <strong class="block"
                    >ERP-2026-042 — Solicitud de compra</strong
                  >
                  <span class="text-color-secondary"
                    >Sistemas · $45,000 MXN</span
                  >
                </div>
                <p-tag
                  value="Pendiente"
              ...
              <p class="m-0 text-color-secondary">
                El drawer inferior es ideal para mobile como alternativa a un
                modal completo.
              </p>
            </div>
          </p-drawer>
              ...
              <p-popover #actionsPopover>
                <div class="flex flex-column gap-1" style="min-width:180px">
                  @for (a of popoverActions; track a.label) {
                    <button
                      class="flex align-items-center gap-2 p-2 border-round w-full border-none bg-transparent cursor-pointer text-left hover:surface-hover"
        ...
        <p-card header="Tooltips — pTooltip">
          ...
          <div class="grid">
            ...
            <div class="col-12 md:col-6">
              <span
                class="text-xs font-bold text-color-secondary uppercase mb-3 block"
                style="letter-spacing:.06em"
                >Posiciones</span
              >
              <div class="flex flex-wrap gap-2">
              ...
              <div class="col-12 md:col-4">
                <div class="flex align-items-start gap-2">
                  <app-icon
                    [icon]="r.icon"
                    [style.color]="r.color"
                    class="text-lg flex-shrink-0 mt-1"
                  />
                  <div>
...
export class WebOverlays {
  ...
  readonly drawerNav = [
    { label: "Dashboard", icon: "icon.view-dashboard" },
    { label: "Solicitudes", icon: "icon.file-document" },
    { label: "Aprobaciones", icon: "icon.check-circle" },
    { label: "Reportes", icon: "icon.chart-bar" },
    { label: "Configuración", icon: "icon.cog" },
  ];

  readonly popoverActions = [
    { label: "Ver detalle", icon: "icon.magnify" },
    { label: "Exportar PDF", icon: "icon.file-pdf-box" },
    { label: "Duplicar", icon: "icon.content-copy" },
    { label: "Eliminar", icon: "icon.trash-can", color: "var(--ds-danger)" },
  ];
  ...
  deleteRecord() {
    ...
    this.msgSvc.add({
      severity: "success",
      summary: "Eliminado",
      detail: "ERP-2026-042 fue eliminado.",
      life: 3000,
    });
  }
