...
describe('ProveedorForm', () => {
  ...
  it('should load select items and existing item on init when id is provided', async () => {
    ...
    mockApiResponseS.onGetSelectItem.mockResolvedValue([{ value: 1, label: 'Cat1' }]);
    mockApiResponseS.onGetEnumSelectItem.mockResolvedValue([{ value: 1, label: 'Type1' }]);
    ...
    expect(mockApiResponseS.onGetEnumSelectItem).toHaveBeenCalledWith('EServiceType');
  ...
  it('should validate RFC when length > 5', () => {
    ...
    expect(mockApiResponseS.onGetList).toHaveBeenCalledWith('Providers/ValidarRfc/ABCDEF/cust-123');
  });
