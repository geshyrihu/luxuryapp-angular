...
describe('PendingMinutes', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.overrideComponent(PendingMinutes, { set: { template: '<div>Mock</div>', imports: [] } });
  ...
  it('should call onLoadData when customerId is set via effect', () => {
    ...
    expect(mockApiResponseS.onGetList).toHaveBeenCalledWith('Reports/PendingMinutes/cust-123');
  });
  ...
  it('onLoadData should reset reportData to null before loading', async () => {
    component.reportData.set({ pendings: [], customer: {}, administrador: [] } as any);
