...
export default class LedgerViewer {
  ...
  async onSearch() {
    ...
    try {
      ...
      const res = await this.apiResponseS.onGetItem<FinancialLedgerEntryDTO[]>(url);
      let entries = res ?? [];

      if (this.eventTypeCtrl.value !== null) {
        entries = entries.filter((e) => e.eventType === this.eventTypeCtrl.value);
      }
  ...
  eventTypeClass(type: EFinancialEventType): string {
    if ([EFinancialEventType.RecepcionPago, EFinancialEventType.AplicacionPago, EFinancialEventType.EmisionNotaCredito].includes(type)) {
      return 'bg-green-100 text-green-800';
    }
    if ([EFinancialEventType.ReversoPago, EFinancialEventType.RechazoPago, EFinancialEventType.CancelacionPago].includes(type)) {
