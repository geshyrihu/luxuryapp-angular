...
export class ReportBuilder implements OnInit {
  ...
  protected livePreviewS = inject(LivePreviewService);
  ...
  private async cargar(id: string) {
    ...
    if (d) {
      ...
      this.previewYear.set(d.body.columns?.find((x) => x.dataSource !== "budget")?.year
  ...
  async cargarCatalogo() {
    ...
    const data = await this.catalogS.getFlat(
      this.customerIdS.customerId()!,
      year,
      this.empresaAspel(),
    );
  ...
  previewRowsLeft(rows: ICanvasRow[]) {
    return rows.slice(0, Math.ceil(rows.length / 2));
  }

  previewRowsRight(rows: ICanvasRow[]) {
  ...
  async guardar() {
    ...
    const result = this.id()
      ? await this.api.onPut<IReportDefinition>(
          `dynamic-reports/${this.id()}`,
          dto,
        )
      : await this.api.onPost<IReportDefinition>(`dynamic-reports`, dto);
  ...
  aplicarEjemplo(tipo: "egp" | "balance") {
    if (tipo === "egp") {
      this.form.patchValue({
        name: "Estado de Ganancias y Pérdidas (Ejemplo)",
