...
import { ReportFilterService } from "../contabilidad-online/services/financial-report-filter.service";
...
export class EspejoAspelFull {
  ...
  filasPorGrupo = computed(() => {
    ...
    for (const grupo of data.grupos) {
      ...
      for (const n1 of grupo.cuentasNivel1) {
        filas.push(this.crearFila(n1.numCta, n1.nombre, 1, n1.naturaleza, false,
          n1.saldoInicial, n1.totalesCargo, n1.totalesAbono, n1.totalesPresupuesto,
          grupo.codigo, grupo.nombre));

        for (const n2 of n1.subCuentas) {
          filas.push(this.crearFila(n2.numCta, n2.nombre, 2, n2.naturaleza, false,
            n2.saldoInicial, n2.totalesCargo, n2.totalesAbono, n2.totalesPresupuesto,
            grupo.codigo, grupo.nombre));

          for (const n3 of n2.detalle) {
            filas.push(this.crearFila(n3.numCta, n3.nombre, 3, n3.naturaleza, false,
              n3.saldoInicial, n3.cargos, n3.abonos, n3.presupuesto,
              grupo.codigo, grupo.nombre));
          }
        }
      }

      filas.push(this.crearFila("", `TOTAL ${grupo.nombre}`, "grupo", "", true,
        grupo.saldoInicial, grupo.totalesCargo, grupo.totalesAbono, grupo.totalesPresupuesto,
        grupo.codigo, grupo.nombre));
  ...
  filasFiltradasPorGrupo = computed(() => {
    ...
    for (const [codigo, filas] of base) {
      const nivelMax = nivelesVisibles[codigo] ?? 3;
      const texto = (busquedas[codigo] ?? "").toLowerCase().trim();
      let filtradas = filas;

      if (nivelMax < 3) {
        filtradas = filtradas.filter(f =>
          f.nivel === "grupo" || (f.nivel as number) <= nivelMax
        );
      }
  ...
  private crearFila(
    ...
    return { numCta, nombre, nivel: nivel as any, naturaleza, esTotal, saldoInicial, cargos, abonos, presupuesto, grupoCodigo, grupoNombre };
  }
  ...
  irAGrupo(codigo: string) {
    document.getElementById(`grupo-${codigo}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
