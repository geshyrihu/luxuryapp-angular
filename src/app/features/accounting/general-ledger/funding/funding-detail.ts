...
import { CreateOrdenCompraWizard } from "src/app/features/purchasing/pr/purchase-order/components/create-orden-compra-wizard/create-orden-compra-wizard";
import { PaymentVoucherModal } from "src/app/features/purchasing/pr/purchase-order/components/payment-voucher-modal/payment-voucher-modal";
import { CreateOrdenCompra } from "src/app/features/purchasing/pr/purchase-order/create-orden-compra";
import { OrdenCompraDatosPago } from "src/app/features/purchasing/pr/purchase-order/forms/orden-compra-datos-pago";
...
const tipoGastoEmojis: { [key: number]: string } = {
  ...
  [ETipoGasto.CajaChica]: "ü™ô",
  [ETipoGasto.Extraordinario]: "‚ú®",
  [ETipoGasto.Devoluciones]: "‚Ü©Ô∏è",
  [ETipoGasto.TarjetaDebito]: "üí≥",
  [ETipoGasto.Proyectos]: "üèóÔ∏è",
...
export class FundingDetail {
   ...
   * Llama a la API usando el nuevo mÈtodo onPatch para persistir el cambio.
  ...
  onPaymentStatusChange(orden: any): void {
    ...
    // 2. Llamamos a nuestro nuevo y flamante mÈtodo onPatch.
    //    Tu servicio ya se encarga de los toasts de carga y Èxito/error.
    this.apiResponseS.onPatch(urlApi, body).then((success) => {
      // 3. Manejamos el caso de error. Si la API falla, 'success' ser· false.
      if (!success) {
        // CR√çTICO! Si la actualizaciÛn fallÛ en el backend,
  ...
  onModalAdd() {
    ...
    this.dialogHandlerS
      .openDialog(
        CreateOrdenCompra,
        { tipoGasto: tipoGastoValue }, // ? °AquÌ est· la magia!
