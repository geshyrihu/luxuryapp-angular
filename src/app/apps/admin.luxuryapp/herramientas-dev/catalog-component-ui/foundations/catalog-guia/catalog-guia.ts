...
export class CatalogGuia {
  ...
  readonly metrics = [
    { label: "Consistencia UI", value: "92%", detail: "Componentes alineados a DS", icon: "icon.tune", tone: "primary" },
    { label: "Densidad ERP", value: "Alta", detail: "Lectura rapida sin perder aire visual", icon: "icon.table", tone: "info" },
    { label: "Mobile ready", value: "AA", detail: "Controles tactiles y secciones apilables", icon: "icon.cellphone", tone: "success" },
  ];

  readonly identityPillars = [
    { title: "Autoridad tranquila", icon: "icon.bank", summary: "LuxuryApp debe sentirse corporativa, confiable y precisa sin parecer pesada.", application: "Azul profundo como firma, superficies limpias y acciones primarias muy claras.", severity: "info" as TagSeverity },
    { title: "Operacion sin friccion", icon: "icon.flash", summary: "El usuario ERP necesita decidir rapido, comparar datos y cerrar tareas.", application: "Tipografia compacta, tablas legibles, labels persistentes y estados visibles.", severity: "success" as TagSeverity },
    { title: "Jerarquia auditables", icon: "icon.shield", summary: "Cada pantalla debe dejar claro que es dato, estado, decision o accion.", application: "Color semantico reservado, maximo una primaria por bloque y danger confirmado.", severity: "warn" as TagSeverity },
  ];

  readonly businessScenarios = [
    { title: "Master-Detail (Complejidad)", description: "Gestion de formularios con lineas dinamicas y calculos en tiempo real.", rule: "Usar botones outlined para acciones secundarias y primary para el cierre del flujo.", icon: "icon.format-list-checks" },
    { title: "Dashboard de Lujo (Identidad)", description: "Uso de Luxury Gold para jerarquizar KPIs financieros y resultados premium.", rule: "Maximo 5% de presencia dorada en pantalla para mantener la elegancia.", icon: "icon.star" },
    { title: "Responsive Total (Omnichannel)", description: "Transicion automatica de tablas densas a vistas de tarjetas en dispositivos moviles.", rule: "Obligatorio implementar app-data-view-mobile en cada listado operativo.", icon: "icon.cellphone" },
  ];
  ...
  readonly buttonRules = [
    { variant: "Primary", usage: "Una accion principal por bloque: guardar, crear, aprobar.", avoid: "No usar para acciones destructivas ni acciones repetidas.", severity: "info" as TagSeverity, cardClass: "h-full border-left-3 border-primary surface-card shadow-1", iconClass: "icon.check-circle text-primary text-xl" },
    { variant: "Secondary", usage: "Acciones de soporte: cancelar, regresar, limpiar filtros.", avoid: "No competir visualmente con la accion principal.", severity: "secondary" as TagSeverity, cardClass: "h-full border-left-3 surface-border surface-card shadow-1", iconClass: "icon.arrow-left text-color-secondary text-xl" },
    { variant: "Danger", usage: "Eliminar, rechazar o revertir informacion sensible.", avoid: "Nunca ubicar sin confirmacion en acciones irreversibles.", severity: "danger" as TagSeverity, cardClass: "h-full border-left-3 border-red-500 surface-card shadow-1", iconClass: "icon.delete text-red-600 text-xl" },
    { variant: "Text", usage: "Acciones de baja jerarquia dentro de toolbars o filas.", avoid: "No usar para el CTA principal de un formulario.", severity: "secondary" as TagSeverity, cardClass: "h-full border-left-3 border-300 surface-card shadow-1", iconClass: "icon.dots-horizontal text-600 text-xl" },
  ];

  readonly componentCatalog = [
    { family: "Botones unificados", selector: "custom-button-*", source: "core/components/web/buttons", useCase: "Acciones estandarizadas. Auto-detectan plataforma.", preferredFor: "CRUD, acciones por fila, guardar, editar, eliminar.", avoidWhen: "Nunca é reemplazaron ios-button-* eliminados.", status: "Usar" as const },
    { family: "Inputs unificados", selector: "custom-input-*-signal", source: "core/components/web/inputs", useCase: "CVA completo. Auto-detectan plataforma.", preferredFor: "Todos los formularios é web y mobile.", avoidWhen: "Nunca é reemplazaron ion-input-* eliminados.", status: "Usar" as const },
