...
import { GoogleCalendarRecurrenceMode } from "src/app/core/interfaces/google-calendar-recurrence-mode.enum";
...
export class GoogleCalendarForm implements OnInit {
  ...
  readonly recurrenceOptions = signal<SelectItemDto[]>([]);
  readonly recurrenceModeOptions = signal<SelectItemDto[]>([
    {
      label: "Mismo dia del mes",
      value: GoogleCalendarRecurrenceMode.DayOfMonth,
    },
    {
  ...
  private async loadRecurrenceOptions() {
    ...
    this.recurrenceOptions.set(
      options.filter((item) => item.value !== Recurrence.Eventual),
    );
  }
  ...
  private applyRecurringState(isRecurring: boolean, emitEvent = true) {
    ...
    if (isRecurring) {
      ...
      if (!recurrenceControl.value) {
        recurrenceControl.setValue(Recurrence.Mensual, { emitEvent: false });
      }

      if (!recurrenceModeControl.value) {
        recurrenceModeControl.setValue(
          GoogleCalendarRecurrenceMode.OrdinalWeekday,
          {
            emitEvent: false,
          },
        );
  ...
  private buildRecurrencePreview(): string {
    ...
    const modeSummary =
      recurrenceMode === GoogleCalendarRecurrenceMode.DayOfMonth
