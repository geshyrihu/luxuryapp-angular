...
import { EmployeeOnboardingChecklistItemDTO } from "src/app/apps/reclutamiento.luxuryapp/expediente-del-empleado/recursos-humanos/employee-file/interfaces/employee-file.interfaces";
...
export class StaffOnboardingChecklistModal {
  ...
  async loadChecklist(): Promise<void> {
    ...
    try {
      ...
      >(OperationRecruitmentEndpoints.employeeFiles.onboardingChecklist(this.employeeId));

      if (!tasks || tasks.length === 0) {
        const initialized = await this.apiResponseS.onPost<
          EmployeeOnboardingChecklistItemDTO[]
        >(
          OperationRecruitmentEndpoints.employeeFiles.initializeOnboardingChecklist(
            this.employeeId,
          ),
          null,
          undefined,
    ...
    try {
      const updated =
        await this.apiResponseS.onPut<EmployeeOnboardingChecklistItemDTO>(
          OperationRecruitmentEndpoints.employeeFiles.updateOnboardingChecklistTask(task.id),
          {
