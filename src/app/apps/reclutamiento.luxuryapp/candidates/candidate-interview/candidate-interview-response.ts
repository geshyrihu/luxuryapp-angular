...
export class CandidateInterviewResponse implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (applicationId) {
      this.loadInterviewResponse(applicationId);
    } else {
    ...
    try {
      const result =
        await this.apiResponseS.onGetItem<CandidateInterviewResponseDto>(
          `recruitment-candidate-applications/${applicationId}/interview-response`,
        );
      if (result) {
  ...
  openFeedbackModal(): void {
    this.dialogHandlerS
      .openDialog(
        ...
        { candidateApplicationId: this.applicationId() },
        "Retroalimentación de entrevista",
        this.dialogHandlerS.sizeLg,
      )
      .then((result: boolean) => {
        if (result) {
          this.loadInterviewResponse(this.applicationId());
        }
      });
  }
