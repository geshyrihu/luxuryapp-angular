...
export class ReportConsumos {
  ...
  readonly medidores = signal<Medidor[]>([]);
  readonly medidorOptions = signal<SelectItemDto[]>([]);
  readonly categoriaOptions = signal<SelectItemDto[]>([]);
  readonly areaOptions = signal<SelectItemDto[]>([]);
  ...
  loadCatalogs() {
    ...
    this.apiResponseS.onGetList<Medidor[]>(urlApi).then((result) => {
