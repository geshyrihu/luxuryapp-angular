...
type ClientRow =
  ...
  | { tipo: 'group' | 'item' | 'total-ingresos' | 'total-gastos' | 'diferencia'; descripcion: string; mes1: number; mes2: number; mes3: number; acum: number };
...
export class EstadoResultadosV2ClienteComponent {
  ...
  readonly rows = computed<ClientRow[]>(() => {
    ...
    for (const clas of d.clasificaciones) {
      ...
      for (const mayor of clas.cuentasMayor ?? []) {
        if (mayor.numeroCuenta === '400-000-000' || mayor.numeroCuenta === '600-000-000') continue;
  ...
  private flatten401Children(mayor: ICuentaMayorDto, wr: (i: number) => number, mes: number): ClientRow[] {
  ...
  private createRow(tipo: ClientRow['tipo'], account: IBaseAccountDto, wr: (i: number) => number, mes: number): ClientRow {
    return {
      tipo,
  ...
  private addTotals(totals: number[], row: ClientRow): number[] {
    ...
    return [totals[0] + row.mes1, totals[1] + row.mes2, totals[2] + row.mes3, totals[3] + row.acum];
  }
