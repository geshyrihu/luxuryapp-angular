...
export class ExcelExportService {
  ...
  async exportResumenPresupuesto(
    ...
    const mkBorder = (bottom?: ExcelJS.BorderStyle): ExcelJS.Borders => ({
      ...
      bottom: {
        style: bottom ?? "thin",
        color: { argb: bottom === "dotted" ? dotClr : bClr },
      },
      diagonal: {},
    });
    ...
    cuentas.forEach((item) => {
      ...
      cPct.value = {
        formula: `IF(O${pRow}=0,IF(Q${pRow}>0,1,0),(Q${pRow}-O${pRow})/O${pRow})`,
      };
      ...
      cProm.value = {
        formula: `IFERROR(AVERAGEIF(C${gRow}:N${gRow},"<>0"),0)`,
      };
      ...
      ws.getCell(pRow, 1).alignment = {
        vertical: "middle",
        horizontal: "left",
      };
      ws.getCell(pRow, 2).alignment = {
        vertical: "middle",
        horizontal: "left",
        wrapText: true,
      };
    ...
    const cfRules = [
      {
        ...
        formulae: [0],
        style: { font: { color: { argb: "FFC00000" }, bold: true } },
        priority: 1,
      },
      {
        type: "cellIs" as const,
    ...
    const pstoFooterData: Record<string, any> = {
      ...
      proposedAmount: sumProposedAmt,
      dif: sumProposedAmt - sumCurrentAmt,
      pct:
        sumCurrentAmt > 0
          ? (sumProposedAmt - sumCurrentAmt) / sumCurrentAmt
          : 0,
    };
    meses.forEach((mes, i) => {
      pstoFooterData[`m_${mes}`] = sumPsto[i];
    });
    ...
    meses.forEach((mes, i) => {
      gastoFooterData[`m_${mes}`] = sumGasto[i];
    });
