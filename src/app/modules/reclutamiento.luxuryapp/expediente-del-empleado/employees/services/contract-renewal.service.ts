...
import { ContractRenewalEvaluationDTO } from "src/app/modules/reclutamiento.luxuryapp/expediente-del-empleado/employees/employees/interfaces/contract-renewal.dto";
...
export class ContractRenewalService {
  ...
  isDataReady = computed(
    () => !this.isLoading() && this.renewals().length >= 0,
  );
  ...
  constructor() {
    effect(() => {
      ...
      if (renewals.length > 0) {
        console.log(
          `[ContractRenewalService] ${renewals.length} renewals loaded`,
        );
      }
    });
  }
  ...
  async loadAll(customerId: string, forceRefresh = false): Promise<void> {
    ...
    try {
      const data = await this._apiResponseS.onGetList<
  ...
  async loadByContract(
    ...
    try {
      const data = await this._apiResponseS.onGetList<
        ContractRenewalEvaluationDTO[]
      >(
        EndpointsRecursosHumanos.HR.ContractRenewal.getByContract(
          employeeWorkContractId,
        ),
      );
      return data ?? [];
    } catch (error) {
  ...
  async getById(id: string): Promise<ContractRenewalEvaluationDTO | null> {
    ...
    try {
      const data =
        await this._apiResponseS.onGetItem<ContractRenewalEvaluationDTO>(
          EndpointsRecursosHumanos.HR.ContractRenewal.getById(id),
        );
      return data ?? null;
    } catch (error) {
      console.error(
        "[ContractRenewalService] Error getting renewal by ID:",
        error,
      );
      return null;
    }
  ...
  async registerDecision(
    ...
    try {
      const response =
        await this._apiResponseS.onPost<ContractRenewalEvaluationDTO>(
      ...
      this.hasError.set(true);
      console.error(
        "[ContractRenewalService] Error registering decision:",
        error,
      );
      return null;
    } finally {
  ...
  async linkPerformanceEvaluation(
    ...
    try {
      const response =
        await this._apiResponseS.onPost<ContractRenewalEvaluationDTO>(
          EndpointsRecursosHumanos.HR.ContractRenewal.linkPerformanceEvaluation(
            evaluationId,
          ),
