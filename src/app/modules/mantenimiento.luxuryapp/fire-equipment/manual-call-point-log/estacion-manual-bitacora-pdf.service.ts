...
export class EstacionManualBitacoraPdfService {
  ...
  async downloadPdf(data: any[], from: Date | null, to: Date | null): Promise<void> {
    ...
    const rows = filtered
      .map(
        ...
        <td class="tc">${this.bool(item.accessibleAndVisible)}</td>
        <td class="tc">${this.bool(item.housingOk)}</td>
        <td class="tc">${this.bool(item.leverOk)}</td>
        <td class="tc">${this.bool(item.glassIntact)}</td>
        <td class="tc">${this.bool(item.mountingSecure)}</td>
        <td class="tc">${this.bool(item.signageOk)}</td>
...
<div class="body-doc">
  <table class="data-table">
    <thead><tr>
      <th>Fecha</th><th class="tc">Hora</th><th class="tc">Accesible</th>
      <th class="tc">Carcasa</th><th class="tc">Palanca</th>
      <th class="tc">Cristal</th><th class="tc">Montaje</th>
      <th class="tc">Señalización</th><th>Observaciones</th>
    </tr></thead>
    <tbody>${rows}</tbody>
  </table>
</div>
${this.htmlPrintS.buildStandardFooter(generatedAt)}
    ...
    this.htmlPrintS.printHtml(html, `Bitacora_Estaciones_Manuales_${generatedAt.getTime()}`);
  }
}
