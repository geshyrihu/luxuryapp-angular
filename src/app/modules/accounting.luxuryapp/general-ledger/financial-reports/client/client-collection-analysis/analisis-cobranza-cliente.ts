...
export class AnalisisCobranzaClienteComponent {
  ...
  readonly dashboardData = computed(() => this.store.dashboardData());
  readonly analysisData = computed(() => this.store.analysisData());
  ...
  constructor() {
    effect(
      () => {
        ...
        const m = this.mes();
        if (cid && yr && m) {
          const day = new Date(yr, m, 0).getDate();
          void this.store.loadFor(cid, yr, m, day);
        }
      },
