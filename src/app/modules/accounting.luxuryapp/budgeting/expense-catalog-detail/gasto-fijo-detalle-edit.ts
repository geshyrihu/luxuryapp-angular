...
export interface IGastoFijoDetalleForm {
  id: FormControl<string | null>;
  catalogoGastosFijosId: FormControl<string | null>;
...
@Component({
  selector: "app-gasto-fijo-detalle-edit",
  templateUrl: "./gasto-fijo-detalle-edit.html",
  imports: [
    ReactiveFormsModule,
...
export class GastoFijoDetalleEdit implements OnInit {
  ...
  cb_unidadMedida: any[] = [];
  form: FormGroup<IGastoFijoDetalleForm> =
    this.formB.group<IGastoFijoDetalleForm>({
      id: new FormControl({ value: this.config.data.id, disabled: true }),
      catalogoGastosFijosId: new FormControl("", Validators.required),
      ...
      ivaAplicado: new FormControl(16, Validators.required),
      retencionIVAPorcentaje: new FormControl(0, Validators.required),
      retencionISRPorcentaje: new FormControl(0, Validators.required),
    });
  ...
  onLoadData() {
    this.apiResponseS
      .onGetItem(Endpoints.CatalogoGastosFijosDetalles.getById(this.id))
      .then((result: any) => {
  ...
  onSubmit() {
    ...
    this.apiResponseS
      .onPut(Endpoints.CatalogoGastosFijosDetalles.update(this.id), this.form.value)
      .then((result: boolean) => {
