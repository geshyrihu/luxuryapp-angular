...
export class RecepcionPipasAguaList {
  ...
  onLoadData() {
    const url = Endpoints.RecepcionPipasAgua.listByCustomer(
  ...
  async onDownloadPdf(item: IRecepcionPipaAgua): Promise<void> {
    ...
    const [loaded, jefes] = await Promise.all([
      ...
      this.apiResponseS.onGetList(
        Endpoints.ResponsablesCliente.jefeMantenimiento(
          this.customerIdS.customerId(),
        ),
      ) as Promise<any[]>,
    ]);
  ...
  <div class="body-doc">
    ...
    <div class="data-grid">
      <div>
        ...
        <div class="data-item"><span class="érmino:</span> ${fmtDate(item.horaTermino)}</div>
      ...
      <div>
        ...
        <div class="data-item"><span class="data-label">és:</span> ${fmtNum(item.nivelCisternaDespues, 0)}%</div>
        ...
        <div class="data-item"><span class="data-label">Costo mí:</span> $${fmtNum(item.costoMetroCubico, 2)}</div>
      </div>
    </div>
    ...
    <div class="section-ías de evidencia</div>
