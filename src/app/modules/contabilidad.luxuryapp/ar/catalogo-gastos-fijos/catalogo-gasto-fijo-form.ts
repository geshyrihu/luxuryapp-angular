...
import { GastoFijoPresupuesto } from "src/app/modules/contabilidad.luxuryapp/budgeting/expense-catalog-budget/gasto-fijo-presupuesto";
import { GastoFijoDetalleEdit } from "src/app/modules/contabilidad.luxuryapp/budgeting/expense-catalog-detail/gasto-fijo-detalle-edit";
import { GastoFijoServicios } from "src/app/modules/contabilidad.luxuryapp/budgeting/expense-catalog-detail/gasto-fijo-servicios";
...
export class CatalogoGastoFijoForm implements OnInit {
  ...
  calcularSubtotal(item: any): number {
    ...
    const retencionIva =
      base * ((Number(item.retencionIVAPorcentaje) || 0) / 100);
    const retencionIsr =
      base * ((Number(item.retencionISRPorcentaje) || 0) / 100);
