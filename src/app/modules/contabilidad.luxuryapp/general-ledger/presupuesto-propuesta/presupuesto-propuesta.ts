...
} from "src/app/modules/contabilidad.luxuryapp/general-ledger/presupuesto-propuesta/interfaces/budget-proposal.model";
import { EquiposList } from "src/app/modules/mantenimiento.luxuryapp/equipos-y-maquinaria/machinery/equipos-list";
  ...
  private calculateAggregateTotals(
    ...
    result.forEach((aggregator) => {
      if (aggregator.esFilaAgrupadora) {
        ...
        aggregator.gastoEnero = children.reduce(
          (sum, c) => sum + (c.gastoEnero || 0),
          0,
        );
        aggregator.gastoFebrero = children.reduce(
          (sum, c) => sum + (c.gastoFebrero || 0),
          0,
        );
        aggregator.gastoMarzo = children.reduce(
          (sum, c) => sum + (c.gastoMarzo || 0),
          0,
        );
        aggregator.gastoAbril = children.reduce(
          (sum, c) => sum + (c.gastoAbril || 0),
          0,
        );
        aggregator.gastoMayo = children.reduce(
          (sum, c) => sum + (c.gastoMayo || 0),
          0,
        );
        aggregator.gastoJunio = children.reduce(
          (sum, c) => sum + (c.gastoJunio || 0),
          0,
        );
        aggregator.gastoJulio = children.reduce(
          (sum, c) => sum + (c.gastoJulio || 0),
          0,
        );
        aggregator.gastoAgosto = children.reduce(
          (sum, c) => sum + (c.gastoAgosto || 0),
          0,
        );
        aggregator.gastoSeptiembre = children.reduce(
          (sum, c) => sum + (c.gastoSeptiembre || 0),
          0,
        );
        aggregator.gastoOctubre = children.reduce(
          (sum, c) => sum + (c.gastoOctubre || 0),
          0,
        );
        aggregator.gastoNoviembre = children.reduce(
          (sum, c) => sum + (c.gastoNoviembre || 0),
          0,
        );
        aggregator.gastoDiciembre = children.reduce(
          (sum, c) => sum + (c.gastoDiciembre || 0),
          0,
        );

        // Sumar presupuestos por mes
        aggregator.presupuestoEnero = children.reduce(
          (sum, c) => sum + (c.presupuestoEnero || 0),
          0,
        );
        aggregator.presupuestoFebrero = children.reduce(
          (sum, c) => sum + (c.presupuestoFebrero || 0),
          0,
        );
        aggregator.presupuestoMarzo = children.reduce(
          (sum, c) => sum + (c.presupuestoMarzo || 0),
          0,
        );
        aggregator.presupuestoAbril = children.reduce(
          (sum, c) => sum + (c.presupuestoAbril || 0),
          0,
        );
        aggregator.presupuestoMayo = children.reduce(
          (sum, c) => sum + (c.presupuestoMayo || 0),
          0,
        );
        aggregator.presupuestoJunio = children.reduce(
          (sum, c) => sum + (c.presupuestoJunio || 0),
          0,
        );
        aggregator.presupuestoJulio = children.reduce(
          (sum, c) => sum + (c.presupuestoJulio || 0),
          0,
        );
        aggregator.presupuestoAgosto = children.reduce(
          (sum, c) => sum + (c.presupuestoAgosto || 0),
          0,
        );
        aggregator.presupuestoSeptiembre = children.reduce(
          (sum, c) => sum + (c.presupuestoSeptiembre || 0),
          0,
        );
        aggregator.presupuestoOctubre = children.reduce(
          (sum, c) => sum + (c.presupuestoOctubre || 0),
          0,
        );
        aggregator.presupuestoNoviembre = children.reduce(
          (sum, c) => sum + (c.presupuestoNoviembre || 0),
          0,
        );
        aggregator.presupuestoDiciembre = children.reduce(
          (sum, c) => sum + (c.presupuestoDiciembre || 0),
          0,
        );
      }
    });
