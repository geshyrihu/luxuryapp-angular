...
import { CobranzaOnlineComposicionReportesModal } from "src/app/modules/cobranza.luxuryapp/cobranza-online/analysis/cobranza-online-composicion-reportes-modal";
import type { CobranzaOnlineAnalysisResponse } from "src/app/modules/cobranza.luxuryapp/cobranza-online/interfaces/cobranza-online-analysis.model";
import type { CobranzaOnlineDashboardResponse } from "src/app/modules/cobranza.luxuryapp/cobranza-online/interfaces/cobranza-online-dashboard.model";
...
export class AnalisisCobranzaClienteComponent {
  ...
  readonly kpis = computed(() => {
    ...
    if (m && m.total > 0) {
      cards.push({
        ...
        subtitle:
          "Abonado: " +
          this.formatMoneda(m.collected) +
          " · Faltante: " +
          this.formatMoneda(m.pending),
    ...
    if (e && e.total > 0) {
      cards.push({
        ...
        subtitle:
          "Abonado: " +
          this.formatMoneda(e.collected) +
          " · Faltante: " +
          this.formatMoneda(e.pending),
    ...
    if (r && r.total > 0) {
      cards.push({
        ...
        subtitle:
          "Abonado: " +
          this.formatMoneda(r.collected) +
          " · Faltante: " +
          this.formatMoneda(r.pending),
    ...
    const filas: BreakdownItem[] = [
      {
        ...
        description: (() => {
          ...
          if (d.cobradoExtraordinariaMes)
            parts.push(
              `extraordinaria ${this.formatMoneda(d.cobradoExtraordinariaMes)}`,
            );
          if (d.cobradoRestaurantMes)
            parts.push(
              `restaurante ${this.formatMoneda(d.cobradoRestaurantMes)}`,
            );
  ...
  get totalFilteredSaldo() {
    return this.filteredRows().reduce(
      (acc: number, curr: any) => acc + curr.saldo,
      0,
    );
  }
  ...
  readonly morosos = computed(() => {
    return (
      this.analysisData()?.morosos.map((a: any) => ({
        id: a.numeroCuenta,
        title: this.cleanName(a.condomino),
        amount: a.saldo,
      })) ?? []
    );
  });
  ...
  readonly judicial = computed(() => {
    return (
      this.analysisData()?.cobranzaJudicial.map((a: any) => ({
        id: a.numeroCuenta,
        title: this.cleanName(a.condomino),
        amount: a.saldo,
      })) ?? []
  ...
  readonly totalCartera = computed(() => {
    return (
      this.totalJudicial() + this.totalMorosos() + this.totalDeudaCorriente()
    );
  });

  // --- Anticipos ---
  private readonly anticipos = computed(
    () => this.analysisData()?.anticipos ?? [],
  );
  ...
  readonly totalAFavor = computed(() =>
    Math.abs(this.analysisData()?.totalAnticipos ?? 0),
  );
