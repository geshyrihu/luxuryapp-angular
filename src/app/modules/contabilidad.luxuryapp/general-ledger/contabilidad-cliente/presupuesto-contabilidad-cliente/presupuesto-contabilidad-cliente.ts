...
} from "src/app/modules/cobranza.luxuryapp/cobranza-online/interfaces/presupuesto-contabilidad.model";
import { PurchaseHistory } from "src/app/modules/contabilidad.luxuryapp/general-ledger/presupuesto-web-aspel/purchase-history";
...
export class PresupuestoContabilidadClienteComponent {
  ...
  private async loadData(customerId: string, year: number, mes: number) {
    ...
    const result = await this.svc.getPresupuestoContabilidad(
      customerId,
      year,
      mes,
    );
