...
export class InventarioEstacionManualPdfService {
  private customToastS = inject(CustomToastService);
  private htmlPrintS = inject(HtmlPrintService);

  async downloadPdf(data: InventarioEstacionManualDto[]) {
      ...
      .map(
        (item, index) => `
          <tr>
            ...
            <td>${this.htmlPrintS.esc(item.stationType)}</td>
    ...
    const html = `<!doctype html><html lang="es"><head><meta charset="UTF-8">
      ...
      ${this.htmlPrintS.buildStandardHeader(logo, "INVENTARIO DE ESTACIONES MANUALES", "", generatedAt, "INVENTARIO")}
    ...
    this.htmlPrintS.printHtml(html, "Inventario-Estaciones-Manuales");
  }
}
