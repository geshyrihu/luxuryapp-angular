...
export class InventarioHidranteQrService {
  private htmlPrintS = inject(HtmlPrintService);

  async downloadQr(item: InventarioHidranteDto): Promise<void> {
    const qrDataUrl = await QRCode.toDataURL(`luxuryapp://inspect/Hydrant/${item.id}`, {
      width: 200,
      margin: 1,
    });
    const html = this.buildPageHtml([this.buildLabelBlock(qrDataUrl, item)]);
    this.htmlPrintS.printHtml(html, `QR-HID-${item.localCode ?? item.location}`);
  }

  async downloadAllQr(items: InventarioHidranteDto[]): Promise<void> {
    ...
    this.htmlPrintS.printHtml(html, "QR-Hidrantes");
  }
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .blue-stripe { height: 3px; background: #1d4ed8; width: 100%; margin-bottom: 2mm; }
    .label-code { font-size: 11pt; font-weight: 800; color: #1d4ed8; text-align: center; letter-spacing: 0.5px; margin-bottom: 1mm; }
...
<body>
  ...
  private buildLabelBlock(qrDataUrl: string, item: InventarioHidranteDto): string {
    ...
    ${code}
    <div class="label-title">HIDRANTE<br>${this.htmlPrintS.esc(item.hydrantType)}</div>
