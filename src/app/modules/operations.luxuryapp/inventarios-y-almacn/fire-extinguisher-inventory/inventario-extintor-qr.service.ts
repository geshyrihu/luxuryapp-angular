...
export class InventarioExtintorQrService {
  private htmlPrintS = inject(HtmlPrintService);

  async downloadQr(item: InventarioExtintorDto): Promise<void> {
    ...
    this.htmlPrintS.printHtml(html, `QR-EXT-${item.localCode ?? item.location}`);
  }

  async downloadAllQr(items: InventarioExtintorDto[]): Promise<void> {
    ...
    this.htmlPrintS.printHtml(html, "QR-Extintores");
  }
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .label-code { font-size: 11pt; font-weight: 800; color: #c9a84c; text-align: center; letter-spacing: 0.5px; margin-bottom: 1mm; }
...
<body>
  ...
  private buildLabelBlock(qrDataUrl: string, item: InventarioExtintorDto): string {
    ...
    ${code}
    <div class="label-title">EXTINTOR<br>${this.htmlPrintS.esc(item.extinguisherType)}</div>
