...
export class InventarioDetectorHumoQrService {
  private htmlPrintS = inject(HtmlPrintService);

  async downloadQr(item: InventarioDetectorHumoDto): Promise<void> {
    const qrDataUrl = await QRCode.toDataURL(`luxuryapp://inspect/SmokeDetector/${item.id}`, {
      width: 200,
      margin: 1,
    });
    const html = this.buildPageHtml([this.buildLabelBlock(qrDataUrl, item)]);
    this.htmlPrintS.printHtml(html, `QR-DET-${item.localCode ?? item.location}`);
  }

  async downloadAllQr(items: InventarioDetectorHumoDto[]): Promise<void> {
    ...
    this.htmlPrintS.printHtml(html, "QR-DetectoresHumo");
  }
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .green-stripe { height: 3px; background: #16a34a; width: 100%; margin-bottom: 2mm; }
    .label-code { font-size: 11pt; font-weight: 800; color: #16a34a; text-align: center; letter-spacing: 0.5px; margin-bottom: 1mm; }
...
<body>
  ...
  private buildLabelBlock(qrDataUrl: string, item: InventarioDetectorHumoDto): string {
    ...
    ${code}
    <div class="label-title">DETECTOR DE HUMO<br>${this.htmlPrintS.esc(item.detectorType)}</div>
