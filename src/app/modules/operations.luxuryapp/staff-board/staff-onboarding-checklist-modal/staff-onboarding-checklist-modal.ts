...
import { EmployeeOnboardingChecklistItemDTO } from "src/app/modules/reclutamiento.luxuryapp/expediente-del-empleado/recursos-humanos/employee-file/interfaces/employee-file.interfaces";
...
export class StaffOnboardingChecklistModal {
  ...
  async loadChecklist(): Promise<void> {
    ...
    try {
      ...
      >(
        OperationRecruitmentEndpoints.employeeFiles.onboardingChecklist(
          this.employeeId,
        ),
      );
    ...
    try {
      const updated =
        await this.apiResponseS.onPut<EmployeeOnboardingChecklistItemDTO>(
          OperationRecruitmentEndpoints.employeeFiles.updateOnboardingChecklistTask(
            task.id,
          ),
          {
