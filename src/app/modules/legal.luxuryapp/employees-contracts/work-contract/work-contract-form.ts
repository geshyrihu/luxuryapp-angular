...
import { EmployeeFileWorkPositionDTO } from "src/app/modules/reclutamiento.luxuryapp/expediente-del-empleado/recursos-humanos/employee-file/interfaces/employee-file.interfaces";
import {
  EContractType,
...
export class WorkContractFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    const data = this.config.data
      ?.item as Partial<EmployeeWorkContractDetailDTO> | null;
    const empId = this.config.data?.employeeId as string;
    this.item.set(data);
    this.isEdit.set(!!data?.id);
  ...
  onSubmit(): void {
    ...
    formData.append(
      "startDate",
      this.dateS.getDateFormat(value.startDate) ?? "",
    );
    if (value.endDate) {
