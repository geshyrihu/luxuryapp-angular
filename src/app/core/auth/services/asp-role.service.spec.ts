import { TestBed } from '@angular/core/testing';
import { BehaviorSubject } from 'rxjs';
import { ApplicationRole } from '../../enums/asp-net-roles.enum';
import { AspRoleService } from './asp-role.service';
import { AuthService } from './auth.service';

describe('AspRoleService', () => {
  let service: AspRoleService;
  let userToken$: BehaviorSubject<any>;

  beforeEach(() => {
    userToken$ = new BehaviorSubject<any>(null);

    TestBed.configureTestingModule({
      providers: [
        AspRoleService,
        {
          provide: AuthService,
          useValue: { userToken$ },
        },
      ],
    });
    service = TestBed.inject(AspRoleService);
  });

  it('should be created', () => {
    expect(service).toBeTruthy();
  });

  it('should return false for all roles when no session exists', () => {
    expect(service.hasRole(ApplicationRole.Comite)).toBe(false);
    expect(service.hasRole(ApplicationRole.SuperUsuario)).toBe(false);
  });

  it('should return true for a role the user has', () => {
    userToken$.next({ roles: [ApplicationRole.Comite] });
    expect(service.hasRole(ApplicationRole.Comite)).toBe(true);
  });

  it('should return false for a role the user does NOT have', () => {
    userToken$.next({ roles: [ApplicationRole.Comite] });
    expect(service.hasRole(ApplicationRole.SuperUsuario)).toBe(false);
  });

  it('should treat SuperUsuario and Direccion as equivalent', () => {
    userToken$.next({ roles: [ApplicationRole.SuperUsuario] });
    expect(service.hasRole(ApplicationRole.SuperUsuario)).toBe(true);
    expect(service.hasRole(ApplicationRole.Direccion)).toBe(true);

    userToken$.next({ roles: [ApplicationRole.Direccion] });
    expect(service.hasRole(ApplicationRole.SuperUsuario)).toBe(true);
    expect(service.hasRole(ApplicationRole.Direccion)).toBe(true);
  });

  it('hasAny() should return true if user has at least one of the roles', () => {
    userToken$.next({ roles: [ApplicationRole.Comite] });
    expect(service.hasAny([ApplicationRole.Comite, ApplicationRole.SuperUsuario])).toBe(true);
  });

  it('hasAny() should return false if user has none of the roles', () => {
    userToken$.next({ roles: [ApplicationRole.Administrador] });
    expect(service.hasAny([ApplicationRole.Comite, ApplicationRole.SuperUsuario])).toBe(false);
  });

  it('roleSignal() should return a reactive signal', () => {
    const sig = service.roleSignal(ApplicationRole.Comite);
    expect(sig()).toBe(false);
    userToken$.next({ roles: [ApplicationRole.Comite] });
    expect(sig()).toBe(true);
  });

  it('should update signals when user session changes', () => {
    userToken$.next({ roles: [ApplicationRole.Comite] });
    expect(service.hasRole(ApplicationRole.Comite)).toBe(true);

    userToken$.next({ roles: [] });
    expect(service.hasRole(ApplicationRole.Comite)).toBe(false);
  });
});









