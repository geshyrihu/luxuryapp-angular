  ...
  template: `
    @if (platform.isMobile()) {
      <ion-item button detail="false" lines="none"
        ...
        style="--background:#fff5f5;--background-activated:#fee2e2;--border-radius:8px;
               --padding-start:10px;--inner-padding-end:14px;--min-height:44px;
               margin-bottom:4px;box-shadow:0 1px 4px rgba(0,0,0,0.07);"
      >
        <div slot="start" style="width:32px;height:32px;border-radius:8px;background:#fecaca;
             display:flex;align-items:center;justify-content:center;margin-right:4px;flex-shrink:0;">
          <ion-icon name="trash-outline" style="font-size:17px;color:#dc2626;" />
        </div>
        <ion-label style="font-weight:600;font-size:13px;color:#dc2626;">
          {{ label() || "Eliminar" }}
        </ion-label>
        <ion-icon name="chevron-forward-outline" slot="end" style="color:#fca5a5;font-size:14px;" />
      </ion-item>
    } @else {
      <button
        [type]="type()"
