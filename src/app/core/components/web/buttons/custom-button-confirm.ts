  ...
  template: `
    @if (platform.isMobile()) {
      <ion-item button detail="false" lines="none"
        ...
        style="--background:#f0fdf4;--background-activated:#dcfce7;--border-radius:8px;
               --padding-start:10px;--inner-padding-end:14px;--min-height:44px;
               margin-bottom:4px;box-shadow:0 1px 4px rgba(0,0,0,0.07);"
      >
        <div slot="start" style="width:32px;height:32px;border-radius:8px;background:#bbf7d0;
          ...
          @if (emoji()) {
            ...
            <ion-icon name="checkmark-outline" style="font-size:17px;color:#15803d;" />
          }
        </div>
        <ion-label style="font-weight:600;font-size:13px;color:#15803d;">
          {{ label() || "Confirmar" }}
        </ion-label>
        <ion-icon name="chevron-forward-outline" slot="end" style="color:#86efac;font-size:14px;" />
      </ion-item>
    } @else {
      <button
        [type]="type()"
