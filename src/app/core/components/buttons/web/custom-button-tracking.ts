...
@Component({
  ...
  template: `
    @if (platform.isMobile()) {
      ...
      >
        <p-overlaybadge
          [value]="badgeCount() && badgeCount()! > 0 ? (badgeCount()! > 99 ? '99+' : badgeCount()) : null"
          severity="danger"
  ...
  styles: [`
    ::ng-deep .custom-overlay-badge .p-overlay-badge {
      width:auto;min-width:18px;height:18px;padding:0 4px;font-size:0.75rem;font-weight:bold;
      border-radius:50%;line-height:18px;box-sizing:border-box;display:flex;
      align-items:center;justify-content:center;top:-6px;right:-6px;
    }
