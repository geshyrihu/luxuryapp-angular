...
@Component({
    ...
    <div class="comment-thread">
      ...
      @if (title()) {
        <h4 class="comment-thread-title">
          ...
          {{ title() }} @if (comments().length > 0) { ({{ comments().length }}) }
        </h4>
      }
          ...
          @for (c of comments(); track c.id) {
            <div class="comment-item">
              ...
              <div class="comment-avatar" [style.background]="avatarBg(c)">
                @if (c.avatarUrl) {
                  <img [src]="c.avatarUrl" [alt]="c.authorName" class="comment-avatar-img" />
                } @else {
              ...
              <div class="comment-bubble">
                <div class="comment-meta">
                  ...
                  @if (c.edited) { <span class="comment-edited">(editado)</span> }
                </div>
  ...
  styles: [`
    ...
    .comment-bubble {
      flex: 1;
      background: var(--ds-bg-elevated, #f1f3ff);
      border-radius: 0 var(--ds-radius-md, 6px) var(--ds-radius-md, 6px) var(--ds-radius-md, 6px);
      padding: 0.625rem 0.875rem;
    }
    .comment-meta { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem; }
    .comment-author { font-size: var(--ds-font-size-help, 0.8125rem); color: var(--ds-text-primary); }
    .comment-time { font-size: var(--ds-font-size-micro, 0.75rem); color: var(--ds-text-muted); }
    .comment-edited { font-size: var(--ds-font-size-micro, 0.75rem); color: var(--ds-text-muted); font-style: italic; }
    .comment-text { font-size: var(--ds-font-size-help, 0.8125rem); color: var(--ds-text-primary); margin: 0; white-space: pre-wrap; }
    /* Reactions */
    .comment-reactions { display: flex; flex-wrap: wrap; gap: 0.25rem; margin-top: 0.375rem; }
    ...
    .comment-reaction-btn:hover { border-color: var(--ds-primary); }
    /* Form */
    .comment-form { display: flex; flex-direction: column; gap: 0.375rem; }
    .comment-textarea {
      width: 100%;
      resize: vertical;
