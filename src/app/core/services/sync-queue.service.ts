...
export const BYPASS_OFFLINE_INTERCEPTOR = new HttpContextToken<boolean>(() => false);
...
export class SyncQueueService {
  ...
  async enqueueRequest(request: SyncRequest): Promise<void> {
    try {
      ...
      this.updateQueueLength();
      this.consoleLogger.custom('📥', '#FFA500', '[SyncQueue] Petición encolada:', request.url);
    } catch (error) {
  ...
  async processQueue(): Promise<void> {
    ...
    this.consoleLogger.custom('🔄', '#00BFFF', `[SyncQueue] Procesando ${queue.length} peticiones...`);
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        ...
        this.consoleLogger.custom('✅', '#32CD32', '[SyncQueue] Sincronizado:', req.url);
      } catch (error) {
        this.consoleLogger.error(`[SyncQueue] Fallo al sincronizar: ${req.url}`, error);
        
        // Si perdemos red en medio del procesamiento, detenemos el proceso
        if (!navigator.onLine) {
           this.consoleLogger.custom('⏸️', '#FFA500', '[SyncQueue] Red perdida. Pausando sincronización.');
           queue = queue.slice(i);
           await localforage.setItem(this.storeName, queue);
           this.updateQueueLength();
           return;
