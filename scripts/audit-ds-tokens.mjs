...
const HARDCODED_COLOR_REGEX =
  /(color|background(-color)?|border(-color)?|fill|stroke):\s*(#[0-9a-fA-F]{3,8}|rgb\([^)]+\)|rgba\([^)]+\)|hsl\([^)]+\)|hsla\([^)]+\))/gi;
...
async function runAudit() {
  ...
  const appFiles = globSync("src/app/**/*.{scss,ts}", {
    ignore: "node_modules/**",
  });
  const styleFiles = globSync("src/styles/**/*.{scss,css}", {
    ignore: "node_modules/**",
  });
  ...
  for (const file of files) {
    ...
    for (const chunk of chunks) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (rootRange && lineNo >= rootRange.from && lineNo <= rootRange.to)
          continue;
        ...
        while ((match = HARDCODED_COLOR_REGEX.exec(line)) !== null) {
          if (isStyles) {
            ...
            console.error(
              `❌ [Token Violation] Color hardcodeado encontrado en ${file}:${lineNo}`,
            );
  ...
  if (stylesFindings.length > 0) {
    ...
    console.error(
      `      este módulo— declarar una capa con alcance y registrarla en`,
    );
    console.error(
      `      COLOR_SOURCE_ALLOWLIST con scope 'root-block' (RN-DS-041).`,
    );
  }
  ...
  for (const file of files) {
    ...
    if (readsStyle && mentionsDsToken && !hasThemeDep) {
      ...
      if (inScope) {
        console.error(`❌ ${msg}`);
        console.error(
          `   💡 Añade un effect() sobre ThemeService.themeMode que repinte (ver RN-DS-040).`,
        );
        totalErrors++;
      } else {
        console.warn(
          `ℹ️  ${msg} (fuera del alcance shared/ui: señal, no falla el sello).`,
        );
      }
    }
  }
  ...
  if (outOfScope.length > 0) {
    ...
    console.warn(
      `\nℹ️  ${outOfScope.length} colores hardcodeados en ${files.size} archivo(s) de src/app/modules/** — FUERA del alcance declarado (§2 del plan maestro).`,
    );
    console.warn(
      `   Se reportan como deuda; no bloquean el sello. Ticket aparte.`,
    );
  }
  ...
  if (totalErrors > 0) {
    ...
    console.error(
      `Por favor, reemplaza los valores hardcodeados por variables del Design System o agrega '// ds-ignore' al final de la línea si es intencional.`,
    );
    process.exit(1);
  } else {
    console.log(
