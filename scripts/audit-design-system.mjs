  ...
  if (isStaged) {
    try {
      ...
      for (const relPath of stagedFiles) {
        ...
        // Solo auditar archivos que caen dentro de client/angular/src/app/modules/
        if (absPath.startsWith(appsRoot)) {
          if (absPath.endsWith(".ts") && !absPath.endsWith(".spec.ts"))
            out.ts.push(absPath);
          else if (absPath.endsWith(".scss") || absPath.endsWith(".css"))
            out.css.push(absPath);
          else if (absPath.endsWith(".html")) out.html.push(absPath);
        }
      }
    } catch (e) {
      console.log(
        "No se pudo obtener la lista de archivos de git (modo --staged).",
        e.message,
      );
    }
    return out;
  ...
  lines.forEach((line, i) => {
    ...
    if (/from\s+["']@ionic\//.test(line)) {
      violations.push({
        file,
        line: i + 1,
        msg: "importa @ionic directamente (usar @ui/*)",
      });
    }
  });
}
  ...
  lines.forEach((line, i) => {
    if (/::ng-deep/.test(line)) {
      violations.push({
        file,
        line: i + 1,
        msg: "uso de ::ng-deep en estilos locales",
      });
    }
    if (/!important/.test(line)) {
      violations.push({
        file,
        line: i + 1,
        msg: "uso de !important en estilos locales",
      });
    }
  });
}
  ...
  lines.forEach((line, i) => {
    if (/\bstyle\s*=\s*["']/.test(line)) {
      violations.push({
        file,
        line: i + 1,
        msg: 'uso de estilos inline (style="...")',
      });
    }
    if (/\[style\.[^\]]+\]\s*=/.test(line) || /\[style\]\s*=/.test(line)) {
      violations.push({
        file,
        line: i + 1,
        msg: "uso de binding de estilos inline ([style]...)",
      });
    }
  });
}
