...
const files = [
  ...
  "D:/repos/luxuryapp-api/client/angular/src/app/features/system/catalogs/catalog-component-ui/pages/foundations/catalog-guia/catalog-guia.html"
];
...
for (const file of files) {
  ...
  content = content.replace(/<ng-template\s+#title\s*>([\s\S]*?)<\/div>/g, '<div class="text-xl font-bold mb-2">$1</div>');
  content = content.replace(/<ng-template\s+pTemplate="title"\s*>([\s\S]*?)<\/div>/g, '<div class="text-xl font-bold mb-2">$1</div>');
  
  content = content.replace(/<ng-template\s+#header\s*>([\s\S]*?)<\/div>/g, '<div class="card-header">$1</div>');
  content = content.replace(/<ng-template\s+pTemplate="header"\s*>([\s\S]*?)<\/div>/g, '<div class="card-header">$1</div>');
