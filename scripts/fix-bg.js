...
// Fix dark buttons
htmlCode = htmlCode.replace(/bg-gray-900 border-1 border-gray-900 border-round-md p-2 w-full flex flex-column align-items-center justify-content-center cursor-pointer hover:bg-gray-800 transition-colors transition-duration-150/g,
...
htmlCode = htmlCode.replace(/class="text-xs text-center line-height-2 text-white"/g, 'class="text-xs text-center line-height-2 text-indigo-700 font-medium"');

// Fix icon color (color:#ffffff -> color:#4338ca)
htmlCode = htmlCode.replace(/style="font-size:1\.5rem;color:#ffffff;"/g, 'style="font-size:1.5rem;color:#4338ca;"');
htmlCode = htmlCode.replace(/style="font-size:1\.25rem;color:#ffffff;"/g, 'style="font-size:1.25rem;color:#4338ca;"');
...
if (trIndex === -1) {
    ...
    if (trIndexLF !== -1) {
        htmlCode = htmlCode.substring(0, trIndexLF) + 
    ...
    htmlCode = htmlCode.substring(0, trIndex) + 
        '<tr\r\n        [class.fila-nivel-2-hoja]="item.nivelCuenta === 2 && !item.esFilaAgrupadora"\r\n        [class.fila-nivel-3-hoja]="item.nivelCuenta === 3 && !item.esFilaAgrupadora"\r\n        [class.fila-deficit]="isDeficit(item)"\r\n        [class.fila-alerta-incremento]="isHighIncrease(item)"\r\n      >' + 
