...
html = html.replace(
  /<i class=\"pi pi-list mr-2\"><\/i>/g,
  '<app-icon icon=\"mdi:format-list-bulleted\" class=\"mr-2\"/>',
);
...
html = html.replace(
  /<i class=\"pi pi-star mr-2\"><\/i>/g,
  '<app-icon icon=\"mdi:star\" class=\"mr-2\"/>',
);
html = html.replace(
  /<i class=\"pi pi-wallet text-gold-600 text-xl\"><\/i>/g,
  '<app-icon icon=\"mdi:wallet\" class=\"text-gold-600 text-xl\"/>',
);
html = html.replace(/icon=\"pi pi-arrow-left\"/g, 'icon=\"mdi:arrow-left\"');
html = html.replace(
  /icon=\"pi pi-exclamation-triangle\"/g,
  'icon=\"mdi:alert\"',
);
html = html.replace(/icon=\"pi pi-filter\"/g, 'icon=\"mdi:filter\"');
html = html.replace(
  /icon=\"pi pi-ellipsis-h\"/g,
...
html = html.replace(
  /<i class=\"pi pi-inbox\"><\/i>/g,
  '<app-icon icon=\"mdi:inbox\"/>',
);
html = html.replace(
  /icon=\"pi pi-external-link\"/g,
  'icon=\"mdi:external-link\"',
);
html = html.replace(
  /<i \[class\]=\"scenario.icon \+ ' text-primary text-2xl'\"><\/i>/g,
  '<app-icon [icon]=\"scenario.icon\" class=\"text-primary text-2xl\"/>',
);
html = html.replace(
  /<i \[class\]=\"rule.iconClass\"><\/i>/g,
  '<app-icon [icon]=\"rule.iconClass\"/>',
);
html = html.replace(
  /<i \[class\]=\"card.iconClass\"><\/i>/g,
  '<app-icon [icon]=\"card.iconClass\"/>',
);
html = html.replace(
  /<i \[class\]=\"metric.icon\"><\/i>/g,
  '<app-icon [icon]=\"metric.icon\"/>',
);
...
ts = ts.replace(
  /iconClass: \"pi pi-check-circle /g,
  'iconClass: \"mdi:check-circle ',
);
ts = ts.replace(
  /iconClass: \"pi pi-arrow-left /g,
...
ts = ts.replace(
  /iconClass: \"pi pi-ellipsis-h /g,
  'iconClass: \"mdi:dots-horizontal ',
);
