#!/usr/bin/env node
/**
 * Migrate system/ and hr/ ion-item/ion-label â†’ ili-list-item.
...
for (const htmlPath of ionItemFiles) {
  ...
  // ---- Migrate ion-item â†’ ili-list-item ----
  ...
  html = html.replace(
    /<ion-item\s+[^>]*?>/g,
    (match) => {
      // Check for button and (click) â€” preserve click
  ...
  // <ion-label class="..."> â†’ remove, keep content
  // <ion-label> â†’ remove
  html = html.replace(/<ion-label[^>]*>/g, "");
  html = html.replace(/<\/ion-label>/g, "");

  // Replace slot="start" â†’ start
  html = html.replace(/slot="start"/g, "start");

  // Replace slot="end" â†’ end
  html = html.replace(/slot="end"/g, "end");

  // Replace <ion-note ...> â†’ <span ...>
  html = html.replace(/<ion-note\s+/g, "<span ");
  html = html.replace(/<\/ion-note>/g, "</span>");

  // Replace <ion-list inset="true" class="m-0"> â†’ just <div class="m-0">
  ...
  // Replace <ion-avatar ...> â†’ <div ...>
  html = html.replace(/<ion-avatar/g, "<div");
  html = html.replace(/<\/ion-avatar>/g, "</div>");

  // Replace <ion-accordion-group ...> â†’ <div ...>
  html = html.replace(/<ion-accordion-group/g, "<div");
  html = html.replace(/<\/ion-accordion-group>/g, "</div>");

  // Replace <ion-accordion ...> â†’ <div ...>
  ...
  write(tsPath, ts);
  migrated++;
  console.log(`  âœ“ ${htmlPath.replace(FEATURES, "").replace(/\\/g, "/")}`);
}
